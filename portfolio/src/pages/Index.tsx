      ...
      {/* HEADER */}
        <header className="fixed top-0 w-full bg-card/80 backdrop-blur-md shadow-card z-50">
          <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <h1 className="font-bold text-lg sm:text-xl text-primary">
              ESHWAR AMARAVADI
            </h1>
            ...
            >
              <span className={`h-[3px] w-6 bg-primary transition ${mobileOpen ? "rotate-45 translate-y-[8px]" : ""}`} />
              <span className={`h-[3px] w-6 bg-primary transition ${mobileOpen ? "opacity-0" : ""}`} />
              <span className={`h-[3px] w-6 bg-primary transition ${mobileOpen ? "-rotate-45 -translate-y-[8px]" : ""}`} />
            </button>
          </div>
                ...
                {["home", "about", "skills", "achievements", "coding-platforms", "projects", "contact"].map((x) => (
                  <a
                    ...
                    className={`px-6 py-3 text-sm font-medium transition
                      ${active === x ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-primary"}
                    `}
                  >
                    {x.replace("-", " ").replace(/\b\w/g, (l) => l.toUpperCase())}
                  </a>
